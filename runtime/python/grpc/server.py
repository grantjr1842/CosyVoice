...
"""
Fun-CosyVoice3 gRPC Server

Provides TTS inference via gRPC for Fun-CosyVoice3-0.5B-2512.
"""
...
class CosyVoiceServiceImpl(cosyvoice_pb2_grpc.CosyVoiceServicer):
    def __init__(self, args):
        ...
        logging.info("Fun-CosyVoice3 gRPC service initialized")
    ...
    def Inference(self, request, context):
        if request.HasField("zero_shot_request"):
            ...
            model_output = self.cosyvoice.inference_zero_shot(
                request.zero_shot_request.tts_text, prompt_text, prompt_speech_16k
            )
            ...
            prompt_speech_16k = torch.from_numpy(
                np.array(
                    np.frombuffer(request.instruct_request.prompt_audio, dtype=np.int16)
                )
            ).unsqueeze(dim=0)
            prompt_speech_16k = prompt_speech_16k.float() / (2**15)
            ...
            logging.warning(
                "Unsupported request type. Only zero_shot, cross_lingual, and instruct are supported for Fun-CosyVoice3."
            )
            return
...
def main():
    ...
    grpcServer.start()
    logging.info("Fun-CosyVoice3 gRPC server listening on 0.0.0.0:{}".format(args.port))
