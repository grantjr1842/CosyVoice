...
class CosyVoiceFrontEnd:
    def __init__(
        ...
        try:
            ...
            # Temporary fix for ttsfrd initialization error "open voice lang map failed"
            # It expects a voice_lang_map.json file with a "voice_lang" key
            map_file = os.path.join(resource_dir, "voice_lang_map.json")
            if os.path.exists(resource_dir) and not os.path.exists(map_file):
                try:
                    with open(map_file, "w") as f:
                        f.write('{"voice_lang": []}')
                except Exception as e:
                    logging.warning(f"Failed to create dummy voice_lang_map.json: {e}")
